...
if(isset($_POST['regAdm_user'])) {
    ...
    if(isset($_FILES['producto_foto']) && $_FILES['producto_foto']['error'] === UPLOAD_ERR_OK) {
        ...
        $result = $objconsultas->registrarProdVend($producto_codigo,$producto_talla,$producto_tipo, $producto_nombre, $producto_precio, $producto_stock, $categoria_id, $usuario_id, $producto_foto, $fecha);
} else {
    echo "<script>alert(No se ha seleccionado ninguna imagen. Se utilizará la imagen predeterminada.)</script>";
    echo "<script>location.href='../../views/Vendedor/consultarProd.php'</script>";
}
?>
